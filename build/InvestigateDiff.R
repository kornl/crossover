setStuffInGlobalEnv <- function(designT) {
  design <<- t(designT)  
  ...
  assign("n.per.seq", n.per.seq, globalenv())
  assign("group.id", group.id, globalenv())
  ...
  assign("Xmat.subj", Xmat.subj, globalenv())
  ## period columns
  Xmat.per<-matrix(0,n.dat,n.per)
  for(i in 1:n.dat){
    Xmat.per[i,per[i]]<-1
  }  
  assign("Xmat.per", Xmat.per, globalenv())
}


# Checking model 1:

design <- structure(c(1L, 2L, 1L, 2L, 1L, 3L, 1L, 2L, 3L, 3L, 2L, 3L), .Dim = c(4L, 3L))
model <- 1
setStuffInGlobalEnv(design)
...
## MODEL 1 - TREATMENTS + FIRST-ORDER ADDITIVE CARRY-OVERS
  ...
  list(Var.trt.pair=Var.trt.pair,Var.car.pair=Var.car.pair,model=model)
