...
<<eval1b, echo=TRUE, eval=TRUE, cache=TRUE, tidy=FALSE>>=
...
# Which seems to compare well in regard to Var.trt.pair (and Var.car.pair.1 but a little worde in Var.car.pair.2) to the reference design from literature:
...
<<attachNameSpace, echo=TRUE, eval=TRUE, results='hide'>>=
...
<<detachNameSpace, echo=TRUE, eval=TRUE, results='hide'>>=
detach("namespace:crossover")
@
...
ArA <- crossover:::infMatrix(rcDesignA, v, model=model)

designB <- designA
p1 <- 1; s1 <- 1
designB[p1,s1] <- 2

rcDesignB <- crossover:::rcd(designB, v, model=model)
ArB <- crossover:::infMatrix(rcDesignB, v, model=model)

n1 <- matrix(0, v, s) # sequences
n2 <- matrix(0, v, p) # periods

for (i in 1:v) {
    for (j in 1:s) {
        n1[i,j] <- sum(designB[,j]==i) # (Or designA?)
    }
    for (j in 1:p) {
        n2[i,j] <- sum(designB[j,]==i) # (Or designA?)
    }
}
...
r <- Vectorize(function(x) { sum(x==designB) })(1:v)

t <- rep(0,v)
t[1] <- (r[1]-1)+s*(u1[1]+1)+p*(u2[1]+1)
for (i in 2:v) {
    t[i] <- r[i]+s*u1[i]+p*u2[i]
}
t <- t/(p*s)
...
g <- sqrt(a1^2+a2^2+2*t[3:v]%*%t[3:v])
l1 <- a1+g
l2 <- a1-g
