...
if (!exists("linkMatrix")) attach(loadNamespace("crossover"), name="namespace:crossover", pos=3, warn.conflicts=FALSE)
...
<<informationMatrixRandomEffects, echo=TRUE>>=
s <- 4
p <- 5
v <- 2
model <- 1
...
with $X$ and $Z$ the fixed and random effects design matrices, $\beta\sim\sN(0,R)$ and $\gamma\sim\sN(0,G)$. Then\footnote{Citation/small prove should be provided.} \[\Var(Y)=ZGZ^T+R.\]
