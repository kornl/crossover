...
<<modelsFromLiterature1, echo=TRUE, eval=TRUE, cache=FALSE, tidy=FALSE>>=
s <- 6; p <- 4; v <- 2
...
<<eval1a, echo=TRUE, eval=TRUE, cache=FALSE, tidy=FALSE>>=
...
<<eval1b, echo=TRUE, eval=TRUE, cache=FALSE, tidy=FALSE>>=
...
<<correct, echo=TRUE, eval=TRUE, cache=FALSE>>=
...
# Which seems to compare well in regard to Var.trt.pair (and not only there) to the reference design from literature:
...
rcDesign
crossover:::rcdMatrix(rcDesign, v=v, model=2) %*% crossover:::linkMatrix(model="Self-adjacency model", v)
