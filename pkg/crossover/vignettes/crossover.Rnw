...
<<TestWithGeneral.carryover, echo=TRUE, eval=TRUE>>=
  ...
  for (model in c(1,2,3,4,5,6,7,8)) {
    ...
    if (model %in% c(2,8)) {      
      var2 <- sum(gco$Var.trt.pair[lower.tri(gco$Var.trt.pair)]) #+ sum(gco$Var.car.pair.1[lower.tri(gco$Var.car.pair.1)]) + sum(gco$Var.car.pair.1[lower.tri(gco$Var.car.pair.1)])
