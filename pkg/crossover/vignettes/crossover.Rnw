...
rcDesign <- crossover:::createRowColumnDesign(design, v=v, model=1)
rcDesign
...
H
# Row-Column-Design: (cf. John et al. 2004, Table II and page 2649f.)
rcDesign <- crossover:::createRowColumnDesign(design, v=v, model=8)
rcDesign
# Design Matrix of Row-Column Design:
...
<<eval1, echo=TRUE, eval=TRUE, cache=TRUE>>=
s <- 6; p <- 4; v <- 2
...
rcDesign <- createRowColumnDesign(design1, v, model=1)
...
<<attachNameSpace, echo=TRUE, eval=TRUE>>=
...
<<TestOfDifferentApproaches, echo=TRUE, eval=TRUE>>=
  ...
  rcDesign <- createRowColumnDesign(design, v, model=1)
...
<<TestWithGeneral.carryover, echo=FALSE, eval=FALSE>>=
  compareApproaches
...
We have functions in R and on the C/C++ level with the same name (in our folowing example \texttt{createRCDesign}) that should be used in R and C/C++ respectively. The R function \texttt{createRCDesign} calls the C function \texttt{createRCDesign2R} which handles the conversion from and to the R structures the SEXP are pointing to. Apart from that we often provide an R function with a suffix \texttt{\_R}, i.e.\ in our example \texttt{createRCDesign\_R}, which is a pure R function and reimplements the same functionality for testing purposes.
