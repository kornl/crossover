...
The catalogue, collected and compiled by Professor Byron Jones, contains 241 designs from the following literature: 
...
This model has further interaction parameters $\gamma_{ij}$. Personally I think some of the overspecified parameters should be dropped, for example $\gamma_{ii}$ - but I'll take a look at this later.
