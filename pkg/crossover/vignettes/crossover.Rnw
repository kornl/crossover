...
I would drop the $0$ column... (and apart from consistency - is this really unproblematic?!)
...
<<SearchStrategy, echo=TRUE, eval=TRUE>>=

set.seed(42)
x <- searchCrossOverDesign(s=9, p=5, v=4, model=4, eff.factor=1)
crossover:::plotSearch(x)

@

This package is written using the R packages Rcpp and RcppArmadillo (\cite{eddelbuettel2011rcpp} and \cite{rcpparmadillo}).
