<<set-parent, echo=FALSE, cache=FALSE, include=FALSE>>=
...
\begin{Def}{Partially balanced incomplete block design with two associate classes}
  A \emph{partially balanced incomplete block design with two associate classes} (or short PBIBD(2) or sometimes even shorter D(2)) is a design that fulfils the following (see \cite{clatworthy1973tables}):
  \begin{itemize}
...
<<bibtexA, results='asis', echo=FALSE>>=
if (!exists("bibCall")) {
