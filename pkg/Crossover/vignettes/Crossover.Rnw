...
In figure \ref{GUI} you can see the following four checkboxes, that allow you to see only specific subsets:
...
Monte-Carlo search for first candidates, followed by parallel hill climbing search, extended by long jumps.
...
The appendices are all work in progress in the moment.
...
\begin{Def}{Variance Balance}
  A design is \emph{variance balanced} if $\Var[\hat\tau_i-\hat\tau_j]$ is constant for all pairs of the $n$ treatment effects $\tau_i$, $\tau_j$, $1\leq i,j\leq n$, $i\neq j$.
\end{Def}

\begin{Def}{Partially balanced incomplete block design with two associate classes}
  A \emph{partially balanced incomplete block design with two associate classes} (or short \emph{PBIBD(2)} or sometimes even more shorter emph{D(2)}) is a design that fulfils the following (see \cite{clatworthy1973tables}):
  \begin{itemize}
    \item There are $b$ blocks of $k$ units each.
    \item There are $v$ treatments and $v>k$, so that not all treatments can occur in a block. Each treatment occurs once in each block and is assigned $r$ times.
    \item Let X be the set of treatments. Then there is a partition of $X\times X$ into three relations, with one relation the identity and the other two relations $R_1$ and $R_2$ fulfilling the following properties:
    \begin{itemize}
      \item A treatment $y$ is called an $i$th associates, $i\in\{1,2\}$, if and only if $(x,y)\in R_i$. Each treatment $x$ has $n_i$ $i$th associates.
      \item For all $i,j,k\in\{1,2\}$ there is a constant numbers $p_{jk}^i$, so that for all $(x,y)\in R_i$ the number of treatments $z$ with $(x,z)\in R_j$ and $(z,y)\in R_k$ is $p_{jk}^i$.
      \item The association scheme is \emph{commutative}, i.e.\ $p_{jk}^i=p_{kj}^i$.
    \end{itemize}
  \item There are two numbers $\lambda_1$, $\lambda_2$ so that $(x,y)\in R_i$, $i\in\{1,2\}$, implies that treatment $x$ and $y$ occur together in exactly $\lambda_i$ blocks.
  \end{itemize}
\end{Def}
